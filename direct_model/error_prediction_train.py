...
from disentanglement.direct_model import direct_model
...
def test(data, original_model,model):
    model.eval()
    original_model.eval()
    if CUDA:
        model.cuda()
    ...
    for b, (message, reallabels) in enumerate(data):

        message = ifcuda(message)
        alltocuda(reallabels)
        batch_size = message.size()[0]
        N += batch_size
        q = model(message)

        p = original_model(message)
        labels = {k:torch.square(p[k]-reallabels[k]) for k in p.keys()}
