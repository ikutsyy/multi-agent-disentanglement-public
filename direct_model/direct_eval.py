...
from disentanglement.direct_model import train_direct, error_prediction_train
from disentanglement.direct_model.direct_model_parameters import BATCH_SIZE
from disentanglement.direct_model.train_direct import get_models, test
...
def multi_checkpoint_eval(results_dir, num, savedir="saved_models/checkpoints", load=True,original_model=None):
    if not load:
        ...
        for x in range(num):
            ...
            if original_model is not None:
                modelname = modelname.replace("checkpoint","error_checkpoint")
                logname = logname.replace("checkpoint","error_checkpoint")
                model,loss_log = error_prediction_train.get_models(savedir, original_model,modelname, logname, data_name=None, load=True)
                loss, r = error_prediction_train.test(testdata,original_model, model)
            else:
...
if __name__ == '__main__':
    checkpointdir = "../direct_model/saved_models/error_checkpoints"
    epochs=10
    original_model,_ = train_direct.get_models("saved_models","pickled_model_large_direct.pkl",
                                             "training_data_large_direct.pkl","train",load=True)
    multi_checkpoint_eval("error_results", num=epochs, savedir=checkpointdir, load=False,original_model=original_model)
