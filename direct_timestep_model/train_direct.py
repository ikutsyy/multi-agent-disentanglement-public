...
def test(data, model, computecat=False):
    ...
    for b, (knowledge, labels) in enumerate(data):
        ...
        errors['obs_agents'].append(
            torch.linalg.norm(q['obs_agents'] - labels['obs_agents'], dim=-1).cpu().detach().numpy())
        ...
        errors['messages'].append(
            torch.linalg.norm(q['messages'] - labels['messages'], dim=-1).cpu().detach().numpy())

        errors['positions'].append(
            torch.linalg.norm(q['positions'] - labels['positions'], dim=-1).cpu().detach().numpy())
        errors['targets'].append(
            torch.linalg.norm(q['targets'] - labels['targets'], dim=-1).cpu().detach().numpy())
...
def train_model(epochs, data_name, checkpoint_path=None,continue_data=None):
    ...
    if continue_data is not None:
        ...
        model = direct_model.DirectPredictor(params['num_agents'],params['num_targets'], params['message_size'], params['obs_size'])
        loss_log = []
        start_from=0
...
if __name__ == '__main__':
    ...
    modelname = "pickled_model_" + 'large' + "_direct.pkl"
    checkpoint_path = os.path.join("saved_models","checkpoints")
    data_name = "large"
    training_data_name = "training_data_" + 'large' + "_direct.pkl"
    load = False
    epochs = 10
